...
impl HLD {
    ...
    fn update_path(&mut self, mut u: usize, mut v: usize, diff: i64) {
        while self.top[u] != self.top[v] {
            if self.depth[self.top[u]] < self.depth[self.top[v]] {
                std::mem::swap(&mut u, &mut v);
            }
            
            self.segment_tree.update(1, 1, self.size[1], self.inn[self.top[u]], self.inn[u], diff);
        ...
        self.segment_tree.update(1, 1, self.size[1], self.inn[u] + 1, self.inn[v], diff);
    }
...
fn main() {
    ...
    for _ in 0..m {
        ...
        if t == "P" {
            ...
            writeln!(stdout, "{}", hld.query_path(u, v)).unwrap();
        }
    }
